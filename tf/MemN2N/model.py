...
class MemN2N(object):
    ...
    def build_memory(self):
        ...
        for h in xrange(self.nhop):
            #uT
            #print ('hid[-1]',self.hid[-1].shape)
            self.hid3dim=tf.reshape(self.hid[-1],[-1,1,self.edim])
            #print ('self.hid3dim',self.hid3dim.shape)
            #print('Ain', Ain.shape)

            # uT * m_i
            Aout=tf.matmul(self.hid3dim,Ain,adjoint_b=True) # self.hid3dim shape [128,1,150] Ain shape [128,100,150]
                                                            #  tf.matmul(self.hid3dim,Ain,adjoint_b=True) 等于先把Ain shape变成 [128,150,100],然后再相乘
                                                            # Aout shape [128,1,100]
            ...
            #print('probs3dim', probs3dim.shape)
            #print('Bin', Bin.shape)
            # o =  sum p_i * c_i
            ...
            #
            Cout=tf.matmul(self.hid[-1],self.C) # u
        ...
        if not self.is_test:
            for idx in xrange(self.nepoch):
                ...
                state={
                    ...
                    'valid_preplexity':math.exp(test_loss)
                }
                print (state)
    ...
    def load(self):
        ...
        ckpt=tf.train.get_checkpoint_mtimes(self.checkpoint_dir)
